...
def get_remote_image(
    _id: str, max_size: tuple[int, int], num_of_tiles: int
) -> Image.Image:
...
def get_image(
    filename: str, max_size: tuple[int, int], num_of_tiles: int
) -> Image.Image:
...
def get_generated_image(max_size: tuple[int, int], num_of_tiles: int):
    """For internal images
...
def tile_scrambler(
    ...
    while hamming_distance(new_positions, positions) != length:
        shuffle(new_positions)
    return zip(new_positions, map(itemgetter(1), tiles))
